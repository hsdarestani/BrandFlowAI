...
test('Persian Content Studio localizes dynamic setup requirements and controls',async({page,request})=>{
 ...
 await expect(page.getByRole('heading',{name:'استودیوی محتوا'})).toBeVisible();
 await expect(page.getByText('ابتدا این مراحل را کامل کنید')).toBeVisible();
...
test('legacy Brand DNA route opens the localized Brand Pulse workspace',async({page,request})=>{
 ...
 await expect(page.getByRole('heading',{name:'پالس برند'})).toBeVisible();
 const text=await page.locator('main.app-content').innerText();
 for(const stale of ['A living knowledge base','Complete Brand Pulse','Add product/service','Brand Pulse builder','Brand basics','Analyze website','Generate Brand Pulse','Cancel','Memory text','Preferred channels'])expect(text).not.toContain(stale);
...
test('authenticated language switching replaces page copy without leaving stale Persian or English UI',async({page,request})=>{
 ...
 await expect(page.getByRole('heading',{name:'Brand Pulse'})).toBeVisible();
