"""Immutable schema snapshot used only by Alembic revision 0001.

Do not import current application models here and do not evolve this module when
new model fields are added. Alembic revision 0001 originally called
``Base.metadata.create_all()`` from the live model registry, which made the
historical migration change behavior whenever application models changed.
This snapshot is the model registry as it existed when production migrations
were introduced in commit f890fb4.
"""
...
class InitialBase(DeclarativeBase):
    pass
...
class Plan(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    name: Mapped[str] = mapped_column(String)
    price_monthly: Mapped[float] = mapped_column(Numeric, default=0)
...
class Organization(Base, TimestampMixin):
    ...
    mode: Mapped[str] = mapped_column(String, default="owner")
    plan_id: Mapped[int | None] = mapped_column(ForeignKey("plans.id"), nullable=True)
    billing_status: Mapped[str] = mapped_column(String, default="trial")
    owner_user_id: Mapped[int | None] = mapped_column(ForeignKey("users.id"))
    suspended_at: Mapped[str | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
class Brand(Base, TimestampMixin):
    ...
    primary_language: Mapped[str] = mapped_column(String, default="de")
    additional_languages: Mapped[list] = mapped_column(JSON, default=list)
    description: Mapped[str] = mapped_column(Text, default="")
    status: Mapped[str] = mapped_column(String, default="onboarding")


class BrandDNA(Base, TimestampMixin):
    ...
    channel_rules_json: Mapped[dict] = mapped_column(JSON, default=dict)
    cta_library_json: Mapped[dict] = mapped_column(JSON, default=dict)
    forbidden_words_json: Mapped[dict] = mapped_column(JSON, default=dict)


class BrandMemoryNote(Base):
    __tablename__ = "brand_memory_notes"
    id: Mapped[int] = mapped_column(primary_key=True)
    brand_id: Mapped[int] = mapped_column(ForeignKey("brands.id"))
    note: Mapped[str] = mapped_column(Text)
    source_type: Mapped[str] = mapped_column(String)
    source_id: Mapped[str | None] = mapped_column(String, nullable=True)
    confidence_score: Mapped[float] = mapped_column(Numeric, default=0.75)
...
class Persona(Base, TimestampMixin):
    ...
    description: Mapped[str] = mapped_column(Text, default="")
    pains: Mapped[list] = mapped_column(JSON, default=list)
    desires: Mapped[list] = mapped_column(JSON, default=list)
...
class BrandRule(Base, TimestampMixin):
    ...
    organization_id: Mapped[int | None] = mapped_column(ForeignKey("organizations.id"), nullable=True)
    brand_id: Mapped[int] = mapped_column(ForeignKey("brands.id"))
    category: Mapped[str] = mapped_column(String, default="voice")
    title: Mapped[str] = mapped_column(String)
    description: Mapped[str] = mapped_column(Text, default="")
    severity: Mapped[str] = mapped_column(String, default="info")
    applies_to_channel: Mapped[str | None] = mapped_column(String, nullable=True)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
...
class ChannelAccount(Base, TimestampMixin):
    ...
    connection_status: Mapped[str] = mapped_column(String, default="mock_connected")
    capabilities_json: Mapped[dict] = mapped_column(JSON, default=dict)
    credentials_encrypted_json: Mapped[dict] = mapped_column(JSON, default=dict)
...
class CalendarItem(Base, TimestampMixin):
    ...
    goal: Mapped[str] = mapped_column(String, default="awareness")
    funnel_stage: Mapped[str] = mapped_column(String, default="top")
    content_type: Mapped[str] = mapped_column(String, default="post")
    channels_json: Mapped[list] = mapped_column(JSON, default=list)
    language: Mapped[str] = mapped_column(String, default="en")
    persona_id: Mapped[int | None] = mapped_column(nullable=True)
    pillar_id: Mapped[int | None] = mapped_column(nullable=True)
    product_service_id: Mapped[int | None] = mapped_column(nullable=True)
    offer_id: Mapped[int | None] = mapped_column(nullable=True)
    cta: Mapped[str] = mapped_column(String, default="Learn more")
    risk_score: Mapped[float] = mapped_column(Numeric, default=0.1)
...
class ContentDraft(Base, TimestampMixin):
    ...
    media_asset_ids_json: Mapped[list] = mapped_column(JSON, default=list)
    status: Mapped[str] = mapped_column(String, default="draft_ready")
    current_version_id: Mapped[int | None] = mapped_column(nullable=True)
    brand_fit_score: Mapped[float] = mapped_column(Numeric, default=0.86)
    compliance_score: Mapped[float] = mapped_column(Numeric, default=0.9)
    ai_provider: Mapped[str] = mapped_column(String, default="mock")
    ai_model: Mapped[str] = mapped_column(String, default="deterministic-v1")
    created_by_user_id: Mapped[int | None] = mapped_column(ForeignKey("users.id"), nullable=True)
...
class ContentVersion(Base):
    ...
    prompt_id: Mapped[int | None] = mapped_column(nullable=True)
    created_at: Mapped[str] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class ApprovalRequest(Base, TimestampMixin):
    ...
    public_token_hash: Mapped[str] = mapped_column(String, index=True)
...
class ApprovalAction(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    approval_request_id: Mapped[int] = mapped_column(ForeignKey("approval_requests.id"))
    user_id: Mapped[int | None] = mapped_column(ForeignKey("users.id"), nullable=True)
    action: Mapped[str] = mapped_column(String)
    comment: Mapped[str | None] = mapped_column(Text, nullable=True)
    revision_prompt: Mapped[str | None] = mapped_column(Text, nullable=True)
    version_id: Mapped[int | None] = mapped_column(ForeignKey("content_versions.id"), nullable=True)
    save_to_memory: Mapped[bool] = mapped_column(Boolean, default=True)
    source_channel: Mapped[str] = mapped_column(String, default="in_app")
...
class ScheduledPost(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    draft_id: Mapped[int] = mapped_column(ForeignKey("content_drafts.id"))
    channel_account_id: Mapped[int] = mapped_column(ForeignKey("channel_accounts.id"))
    scheduled_at: Mapped[str | None] = mapped_column(DateTime(timezone=True), nullable=True)
    status: Mapped[str] = mapped_column(String, default="scheduled")
    provider_post_id: Mapped[str | None] = mapped_column(String, nullable=True)
    assisted_publish_url: Mapped[str | None] = mapped_column(String, nullable=True)
...
class PublishedPost(Base):
    ...
    draft_id: Mapped[int] = mapped_column(ForeignKey("content_drafts.id"))
    channel_account_id: Mapped[int] = mapped_column(ForeignKey("channel_accounts.id"))
    provider_post_id: Mapped[str] = mapped_column(String)
...
class InsightSnapshot(Base):
    ...
    normalized_scores_json: Mapped[dict] = mapped_column(JSON, default=dict)
...
class WeeklyReport(Base):
    ...
    insights_json: Mapped[dict] = mapped_column(JSON, default=dict)
...
class Campaign(Base, TimestampMixin):
    ...
    channels_json: Mapped[list] = mapped_column(JSON, default=list)
    content_pillars_json: Mapped[list] = mapped_column(JSON, default=list)
    budget: Mapped[float | None] = mapped_column(Numeric, nullable=True)
    owner_user_id: Mapped[int | None] = mapped_column(ForeignKey("users.id"), nullable=True)


class CampaignPlanItem(Base, TimestampMixin):
    __tablename__ = "campaign_plan_items"
    id: Mapped[int] = mapped_column(primary_key=True)
    campaign_id: Mapped[int] = mapped_column(ForeignKey("campaigns.id"))
    title: Mapped[str] = mapped_column(String)
    channel: Mapped[str] = mapped_column(String, default="general")
    content_type: Mapped[str] = mapped_column(String, default="post")
    brief: Mapped[str] = mapped_column(Text, default="")
    suggested_date: Mapped[str | None] = mapped_column(String, nullable=True)
    status: Mapped[str] = mapped_column(String, default="idea")
    linked_content_draft_id: Mapped[int | None] = mapped_column(ForeignKey("content_drafts.id"), nullable=True)
    linked_calendar_item_id: Mapped[int | None] = mapped_column(ForeignKey("calendar_items.id"), nullable=True)
...
class SetupChecklistItem(Base, TimestampMixin):
...
class Asset(Base, TimestampMixin):
    ...
    brand_id: Mapped[int] = mapped_column(ForeignKey("brands.id"))
    name: Mapped[str] = mapped_column(String)
    asset_type: Mapped[str] = mapped_column(String, default="metadata_only")
...
class ManualMetric(Base, TimestampMixin):
...
class AIUsageLog(Base):
    ...
    cost_estimate: Mapped[float] = mapped_column(Numeric, default=0)
    created_at: Mapped[str] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class MessagingConsent(Base):
    ...
    consent_source: Mapped[str] = mapped_column(String, default="manual")
    consented_at: Mapped[str | None] = mapped_column(DateTime(timezone=True), nullable=True)
